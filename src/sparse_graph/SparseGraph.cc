...
/* minimal information is frame with feature keypoints and descriptor, corresponding 3d position */
bool
SparseGraph::readFromColmap(const std::string& mapName, const std::string& databaseName)
{
    boost::filesystem::path databaseFilePath(databaseName);
    boost::filesystem::path pointsFilePath(mapName);
    boost::filesystem::path imageFilePath(mapName);
    //databaseFilePath /= "database.db";
    pointsFilePath /= "points3D.txt";
    imageFilePath /= "images.txt";

    sqlite3 *db;
    int rc;

    rc = sqlite3_open(databaseFilePath.string().c_str(), &db);
    ...
    boost::unordered_map<size_t, FramePtr> frameMap;
    boost::unordered_map<size_t, Point3DFeaturePtr> feature3DMap;

    //read images
    std::ifstream file(imageFilePath.string());
    std::string line;
    std::string item;
    while (std::getline(file, line)) {
        ...
        frameMap[image_id] = boost::make_shared<Frame>();

        // QVEC (qw, qx, qy, qz) - Not used.
        std::getline(line_stream1, item, ' ');
        std::getline(line_stream1, item, ' ');
        std::getline(line_stream1, item, ' ');
        std::getline(line_stream1, item, ' ');
        ...
        std::vector<Point2DFeaturePtr>& features2D = frameMap[image_id]->features2D();
        features2D.clear();

        //descriptors
        cv::Mat descriptors_cv;
        char sqlSentence_format[100];
        sprintf (sqlSentence_format, "SELECT * FROM descriptors WHERE image_id = %d;", image_id);
        const char *sqlSentence = sqlSentence_format;
        sqlite3_stmt *stmt = NULL;
        int result = sqlite3_prepare_v2(db, sqlSentence, -1, &stmt, NULL);


        if (result == SQLITE_OK && sqlite3_step(stmt) == SQLITE_ROW) {
            int64_t name = sqlite3_column_int64(stmt, 0);
            const size_t rows =
                    static_cast<size_t>(sqlite3_column_int64(stmt, 1));
            const size_t cols =
                    static_cast<size_t>(sqlite3_column_int64(stmt, 2));
            Eigen::Matrix<uint8_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> descriptors(rows, cols);
            const size_t num_bytes =
                    static_cast<size_t>(sqlite3_column_bytes(stmt, 3));

            memcpy(reinterpret_cast<char *>(descriptors.data()),
                   sqlite3_column_blob(stmt, 3), num_bytes);
            Eigen::MatrixXf descriptors_float(descriptors.rows(),
                                              descriptors.cols());
            for (Eigen::MatrixXf::Index r = 0; r < descriptors.rows(); ++r) {
                for (Eigen::MatrixXf::Index c = 0; c < descriptors.cols(); ++c) {
                    const float scaled_value = descriptors(r, c) / 512.0;
                    descriptors_float(r, c) = scaled_value;
                }
            }
            eigen2cv(descriptors_float, descriptors_cv);
        }
        else {
            std::clog << "error for selection"<<std::endl;
        }
        ...
        if (!line.empty()) {
            while (!line_stream2.eof()) {
                ...
                feature2d->keypoint().pt.x = point.x();

                std::getline(line_stream2, item, ' ');
                point.y() = boost::lexical_cast<double>(item);
                feature2d->keypoint().pt.y = point.y();
                //descriptor
                feature2d->descriptor() = descriptors_cv.row(features2D.size());
                ...
                std::getline(line_stream2, item, ' ');
                if (item == "-1") {
                    point3D_ids.push_back(-1);
                } else {
                    point3D_ids.push_back(boost::lexical_cast<int64_t>(item));
                }
            }
        }
    ...
    // read points3d
    std::ifstream file_point3d(pointsFilePath.string());

    while (std::getline(file_point3d, line)) {
        StringTrim(&line);
        if (line.empty() || line[0] == '#') {
            continue;
        }
        ...
        std::getline(line_stream, item, ' ');
        point3dfeature->point()[1] = boost::lexical_cast<double>(item);

        std::getline(line_stream, item, ' ');
        point3dfeature->point()[2] = boost::lexical_cast<double>(item);
        ...
        while (!line_stream.eof()) {
            ...
            size_t image_id = boost::lexical_cast<size_t>(item);

            std::getline(line_stream, item, ' ');
            size_t point2D_idx = boost::lexical_cast<size_t>(item);

            point3dfeature->features2D().push_back(frameMap[image_id]->features2D().at(point2D_idx));
            frameMap[image_id]->features2D().at(point2D_idx)->feature3D() = point3dfeature;

        }
    }


    m_frameSetSegments.resize(1);
    size_t frameSetSize = frameMap.size();
    ...
    for (size_t segmentId = 0; segmentId < m_frameSetSegments.size(); ++segmentId)
    {
        ...
        for (size_t frameSetId = 0; frameSetId < segment.size(); ++frameSetId)
        {
            ...
            int count = 0;
            for (auto iter = frameMap.begin(); iter != frameMap.end(); iter++)
            {
                size_t frameId = iter->first;
                frameSet->frames().at(count) = frameMap.at(frameId);
                count++;
            }
        }
    }
