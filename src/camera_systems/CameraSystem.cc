...
bool CameraSystem::readExtrinsicsFromTextFile(const std::string& directory) {
    ...
    std::ifstream ifs(extrinsicPath.c_str());
...
CameraSystem::writePosesToTextFile(const std::string& filename) const
{
    ...
    for (int i = 0; i < m_cameraCount; ++i)
    {
        const Eigen::Matrix4d& globalPose = getLocalCameraPose(i);
...
CameraSystem::writeToXmlFile(const std::string& filename) const
{
    ...
    for (int i = 0; i < m_cameraCount; ++i)
    {
        ...
        eSupplier.append_child(pugi::node_pcdata).set_value("InfrasCal v1.0");
        ...
        if(i==0) continue;
        Eigen::Matrix4d H_prev = m_globalPoses.at(i-1);
        Eigen::Matrix4d H = m_globalPoses.at(i).inverse()*H_prev;

        pugi::xml_node eExtrinsics = eCamera.append_child("extrinsic-parameters_T_cn_cnm1");
...
const int
CameraSystem::getReferenceCameraId()
{
    return m_referenceCameraIdx;
}
...
void
CameraSystem::updateGlobalPoses()
{
    if(m_referenceCameraIdx == -1)
    {
        return;
    }
    for (int i = 0; i < m_cameraCount; ++i) {
        setLocalCameraPose(i, m_localPoses.at(i));
    }
}
}
