...
bool TemplatedVocabulary<TDescriptor,F>::loadFromBinFile(const std::string &filename) {
  ...
  while (!f.eof()) {
    ...
    m_nodes[nid].descriptor = std::vector<float>(F::L);
    memcpy(m_nodes[nid].descriptor.data(), buf+4, F::L* sizeof(float));
    m_nodes[nid].weight = *(float*)(buf+4+F::L* sizeof(float));
    if (buf[8+F::L* sizeof(float)]) { // is leaf
...
void TemplatedVocabulary<TDescriptor,F>::saveToBinFile(const std::string &filename) const {
  ...
  float _weight;
  unsigned int size_node = sizeof(m_nodes[0].parent) + F::L*sizeof(float) + sizeof(_weight) + sizeof(bool);
  ...
  for(size_t i=1; i<nb_nodes;i++) {
    ...
    f.write((char*)(&(node.descriptor[0])), F::L*sizeof(float));
