...
void
LocationRecognition::setup(const SparseGraph& graph, const std::string& vocFilename)
{
    m_frameTags.clear();
    m_frames.clear();
    m_frameMap.clear();

    Sift128Vocabulary voc;
    // voc.load("surf64.yml.gz");

    voc.loadFromBinFile(vocFilename);
...
void
LocationRecognition::knnMatch_maploc(const FrameConstPtr& frame, int k,
                              std::vector<FrameTag>& matches) const
{
    DBoW2::QueryResults ret;
    m_db.query(frameToBOW(frame), ret, 0);
