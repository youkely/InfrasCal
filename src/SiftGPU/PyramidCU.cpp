...
void PyramidCU::GenerateFeatureList()
{
    ...
    FOR_EACH_OCTAVE(i, reverse)
	{
        ...
        FOR_EACH_LEVEL(j, reverse)
		{
			...
			int tmp_idx = i * param._dog_level_num + j;
			//std::cout << "Extract features from octave " << i << ", level " << j << " (idx " << tmp_idx << ")\n";
			...
			//if(ProgramCU::CheckErrorCUDA("PyramidCU::GenerateFeatureList()4")) {SetFailStatus(); return;}
