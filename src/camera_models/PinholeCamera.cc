...
void PinholeCamera::centerize(Eigen::Vector2d& p_pixel) const
{
    p_pixel(0) = p_pixel(0) - mParameters.cx();
    p_pixel(1) = p_pixel(1) - mParameters.cy();
}
...
void
PinholeCamera::getPrinciplePoint(std::vector<double>& cxcy) const
{
    Parameters params = getParameters();
    cxcy.at(0) = params.cx();
    cxcy.at(1) = params.cy();
...
void
PinholeCamera::setFocalLength(const std::vector<double> &fxfy)
{
    Parameters params = getParameters();
    params.fx() = fxfy.at(0);
    params.fy() = fxfy.at(1);
