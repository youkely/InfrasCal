...
void
OCAMCamera::centerize(Eigen::Vector2d& p_pixel) const
{
    p_pixel(0) = p_pixel(0) - mParameters.center_x();
    p_pixel(1) = p_pixel(1) - mParameters.center_x();
}
...
void
OCAMCamera::setPrinciplePoint(const std::vector<double>& cxcy)
{
    Parameters params = getParameters();
    params.center_x() = cxcy.at(0);
    params.center_y() = cxcy.at(1);
    setParameters(params);
}

void
OCAMCamera::getPrinciplePoint(std::vector<double>& cxcy) const
{
    Parameters params = getParameters();
    cxcy.at(0) = params.center_x();
    cxcy.at(1) = params.center_y();
}
