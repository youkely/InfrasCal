...
void
Camera::setFocalLength(const std::vector<double>& fxfy)
{
}
...
double
Camera::radialReprojectionError(const Eigen::Vector3d& P,
                          const Eigen::Quaterniond& camera_q,
                          const Eigen::Vector3d& camera_t,
                          const Eigen::Vector2d& observed_p) const
{
    ...
    Eigen::Vector2d p1_pred(P_cam[0]/P_cam[2], P_cam[1]/P_cam[2]);
    double p1_pred_norm = p1_pred.norm();
    p1_pred(0) = p1_pred(0)/p1_pred_norm;
    p1_pred(1) = p1_pred(1)/p1_pred_norm;
    Eigen::Vector2d p1_pixel(observed_p(0),observed_p(1));
    centerize(p1_pixel);
