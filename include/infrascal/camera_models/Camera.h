...
public:
    ...
    // move the pixel into the centerized coordinate
    virtual void centerize(Eigen::Vector2d& p_pixel) const = 0;
    ...
    virtual void setPrinciplePoint(const std::vector<double>& cxcy) = 0;
    virtual void getPrinciplePoint(std::vector<double>& cxcy) const = 0;
    virtual void setFocalLength(const std::vector<double>& fxfy);
    ...
    double radialReprojectionError(const Eigen::Vector3d& P,
                             const Eigen::Quaterniond& camera_q,
                             const Eigen::Vector3d& camera_t,
                             const Eigen::Vector2d& observed_p) const;
